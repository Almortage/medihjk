...
async def start(message: Message):
    ...
    if subscribe:
        ...
        await app.reply_to(
            message,
            caption if others.get("subscribe") is None else others.get("subscribe")
        )
    ...
    markup = Keyboard([
        [
            Button("- المطور -", "BENN_DEV.t.me")
        ],
        [
            Button("- قنواتك -", callback_data="my_channels"),
            Button("- اضف قناه -", callback_data="users_add_channel"),
            Button("- حذف قناه -", callback_data="users_remove_channel")
        ],
        [
...
async def add_comment(response ,chat_id, message_id, user_id):
    ...
    markup = Keyboard([
        [
            Button("- لنشر تعليقك -", url=f"https://t.me/{bot.username}?start=message_{copied.message_id}_chat_{chat_id}")
        ]
    ])
    caption = (str(response.caption) if any([response.animation, response.audio, response.photo, response.video, response.video_note, response.voice, response.dice, response.document ]) else str(response.text))
    comment_caption = f"{caption}\n\n- [صاحب التعليق](tg://openmessage?user_id={user_id})"
    if "None" in caption: caption = caption.split("None")[-1]
    if any([ response.animation, response.audio, response.photo, response.video, response.video_note, response.voice, response.dice, response.document]):
        try:
            await app.edit_message_caption(
                ...
                parse_mode="Markdown",
                reply_markup=markup# @BENN_DEV & @BENfiles
           )
...
@app.callback_query_handler(
    func = lambda callback: callback.data == "help")
...
async def my_channels(callback: CallbackQuery):
    ...
    if len(user_channels) == 0:
        await app.answer_callback_query(callback.id, "ليس لديك أي قنوات مضافه بعد.", show_alert=True)
        return
    markup = []# @BENN_DEV & @BENfiles
    for username in user_channels:
        channel = await app.get_chat(username)
        name = channel.title
        typeness = channel.type
        url = (await app.create_chat_invite_link(username)).invite_link if type(username) == type(1) else f"{username.split('@')[-1]}.t.me"
        name_btn = [Button(name, url)]
        info_btn = [Button(username, callback_data="username"), Button(typeness,  callback_data="typeness")]
    ...
    await app.edit_message_text(
        chat_id=user_id,
        message_id = callback.message.id, 
        text = caption,# @BENN_DEV & @BENfiles
...
async def users_add_channel(callback: CallbackQuery):
    ...
    if len(user_channels) >= 3:
        await app.answer_callback_query(callback.id, "لقد وصلت إلى الحد الأقصى من القنوات (3).", show_alert=True)
        return
    try:
        response = await listener.listen_to(callback, "- قم بإرسال معرف القناه مبدوءًا ب @ اذا كانت القناه عامه.\n- أو قم بإرسال رابط الدعوه إذا كانت القناه خاصه.\n- سيتم إلغاء استلام القناه تلقائيا بعد 30 ثانبه.", timeout=30)
    except TimeOut:
        await app.send_message(
            chat_id=user_id,
            text="انتهى وقت استلام القناه."
        )
        return
    ...
    if channel is str:# @BENN_DEV & @BENfiles
        try:
...
async def removing_user_channel(callback: CallbackQuery):
    ...
    users[str(user_id)]["channels"].remove(int(channel) if channel.startswith("-") else channel)
    ...
    await app.edit_message_text(
        ...
        reply_markup=Keyboard([[Button("- العوده -", callback_data="users_start")]])
    )
...
@app.callback_query_handler(
    func = lambda callback: callback.data in users[str(callback.from_user.id)]["settings"])
async def change_attr(callback: CallbackQuery):
    user_id = callback.from_user.id
    data = callback.data
    users[str(user_id)]["settings"][data] = True if not users[str(user_id)]["settings"][data] else False
    user_channels = users[str(user_id)]["channels"]
    try:
        for channel in user_channels:
            users_channels[str(channel)]["settings"][data] = True if not users_channels[str(channel)]["settings"][data] else False
    except:
        pass
    write(users_channels_db, users_channels)
    write(users_db, users)
...
async def change_emo(callback: CallbackQuery):
    ...
    if response.text == current:
        await app.edit_message_text(
            chat_id=user_id,
            message_id=response.output.id, 
            text=f"{'الإيموجي' if data == 'change_emo' else 'الحقوق'} الذي أرسلته مضاف بالفعل."
        )
        return
    users[str(user_id)]["emo" if data == "change_emo" else "rights"] = response.text
    user_channels = users[str(user_id)]["channels"]
    for channel in user_channels:
            users_channels[str(channel)]["emo" if data == "change_emo" else "rights"] = response.text
    write(users_db, users)
    write(users_channels_db, users_channels)
    await app.edit_message_text(
        chat_id=user_id,
...
@app.callback_query_handler(
    func = lambda callback: callback.data == "post")
...
async def send_message(callback: CallbackQuery):
    user_id = callback.from_user.id
    markup = callback.message.reply_markup.keyboard# @BENN_DEV & @BENfiles
...
async def editor(response, chat_id, message_id, user_id = False):
    user_rights = users[str(user_id)]["rights"] if user_id else users_channels[str(chat_id)]["rights"]
    user_emo = users[str(user_id)]["emo"] if user_id else users_channels[str(chat_id)]["emo"]
    user_settings = users[str(user_id)]["settings"] if user_id else users_channels[str(chat_id)]["settings"]
        ...
        if user_settings["add_emo"]:
            markup = Keyboard([
                [
                    ...
                    Button(channel_title, url=f"{chat_id.split('@')[-1]}.t.me" if isinstance(chat_id, str) else (await app.create_chat_invite_link(chat_id)).invite_link)
                ]
            ])
        else:
            markup = Keyboard([[Button(channel_title, url=f"{chat_id.split('@')[-1]}.t.me" if isinstance(chat_id, str) else (await app.create_chat_invite_link(chat_id)).invite_link)]])
        await app.edit_message_reply_markup(
            chat_id=chat_id,# @BENN_DEV & @BENfiles
            message_id=message_id,
            reply_markup=markup
        )
    if user_settings["comment_button"]:
        ...
        if all([user_settings["add_emo"], user_settings["channel_button"]]):
            markup = Keyboard([
                ...
                [
                    Button("اضف تعليق ♡.", url=f"https://t.me/{bot.username}?start=message_{message_id}_chat_{chat_id if isinstance(chat_id, int) else chat_id.split('@')[-1]}")
                ]
            ])
...
@app.message_handler(regexp=r"^(المطور|المبرمج|بن)$")
async def dev(message: Message):
    ...
    caption = user_bio
    markup = Keyboard([[Button(nickname, url=f"{username}.t.me")]]) # @BENN_DEV & @BENfiles
...
async def vote(callback: CallbackQuery):
    ...
    info = data.split("_")[1:]
    try:
        if (await app.get_chat_member("@" + channel_user, user_id)).status not in ["member", "creator", "administrator"]:
            await app.answer_callback_query(callback.id, "يجب عليك الإنضمام أولًا لتتمكن من التصويت.🚫", show_alert=True)
            return
    except:
        ...
    if user_id in votes[str(chat_id) if channel_user is None else f"@{channel_user}"][info[-1]]:
        await app.answer_callback_query(callback.id, "لقد قمت بالتصويت من قبل.🚫", show_alert=True)
        return
    votes[str(chat_id) if channel_user is None else f"@{channel_user}"][info[-1]].append(user_id)
    write(votes_db, votes)
    markup = callback.message.reply_markup.keyboard
    emo = markup[0][0].text.split()[-1]
    await app.answer_callback_query(callback.id, f"You reacted to this {emo}")
    text = f"{int(info[0]) +1} {emo}"# @BENN_DEV & @BENfiles
...
@app.channel_post_handler(
    content_types=["text", "photo", "animation", "video", "sticker", "document", "dice", "video_note", "audio", "voice"])
...
def channels_markup(user_id):
    ...
    markup = Keyboard([
        [
            Button(
                "- زر رابط القناه ✅️ -" if user_setting["channel_button"] else "- زر رابط القناه ❌️ -", 
                callback_data="channel_button"),
        ],
        [
            Button("- تغيير الإيموجي -", callback_data="change_emo"),
            Button(
                "- إضافة ايموجي ✅️ -" if user_setting["add_emo"] else "- إضافة ايموجي ❌️ -", 
                callback_data="add_emo"),
        ],
        [
            Button("- تغيير الحقوق -", callback_data="change_rights"),
            Button(
                "- إضافة حقوق ✅️ -" if user_setting["add_rights"] else "- إضافة حقوق ❌️ -", 
                callback_data="add_rights"),
        ],
        [
            Button(
                "- زر التعليقات ✅️ -" if user_setting["comment_button"] else "- زر التعليقات ❌️ -", 
...
def read(fp):
    if not os.path.exists(fp):
        with open(fp, "w") as file:
            json.dump({} if fp not in ["channels.json", "banned.json", "admins.json"] else [], file, indent=2)
...
users_db = "users.json"
channels_db = "channels.json"# @BENN_DEV & @BENfiles
